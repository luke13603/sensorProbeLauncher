#include "headers.h"
constexpr uint8_t IN1Y = 8, IN2Y = 7, IN1P = 9, IN2P = 6; //pin defs, 1 is speed, 2 is direction
constexpr uint8_t CLK_PITCH = 5, DT_PITCH = 4, DT_YAW = 3, CLK_YAW = 2, DIRECTION_CW = 1, DIRECTION_CCW = -1;  
...
static int counterP = 0, prevPitchCLK = 0, counterY = 0, prevYawCLK = 0;
...
float getPitchAngle(){float a=wrap360(counterP*degreesPerCountPITCH);return a;}
float getYAWAngle(){float a=wrap360(counterY*degreesPerCountYAW);return a;}
...
void moveUp(){digitalWrite(IN1P, LOW); analogWrite(IN2P, 150);}
void moveDown(){digitalWrite(IN1P, HIGH); analogWrite(IN2P, 150);}
void moveLeft(){digitalWrite(IN1Y, HIGH); digitalWrite(IN2Y, LOW);}
void moveRight(){digitalWrite(IN1Y,LOW); digitalWrite(IN2Y, HIGH);}
void stopAll(){
  digitalWrite(IN1Y, LOW); digitalWrite(IN2Y, LOW); 
